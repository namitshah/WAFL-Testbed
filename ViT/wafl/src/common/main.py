...
class NetworkTrafficUtils:
    ...
    def save_traffic_metrics(
        ...
        Tcpdump rotates through a set of 10 200 MB .pcap files.
            ...
            for fileID in range(10):
                ...
                if os.path.exists(file_curr):
                    ...
                    try:
                        ...
                        NetworkTrafficUtils.logger.info(f"Failed to read pcap file '{file_curr}': {e}")
...
class ModelLearningUtils:
    ...
    def __init__(
        ...
        noniid_filter_path = "./dataset/non-IID_filter"
        ...
        mean_t = torch.load("./dataset/test_mean_and_std/test_mean.pt")
        std_t = torch.load("./dataset/test_mean_and_std/test_std.pt")
        self.logger.info("Loading of mean and std in test data finished")
        ...
        self.logger.info("Test tansformations finished.")
        ...
        train_data = MyGPUdataset(
            train_path, device, len(classes), indices[agent_index], pre_transform=transforms.Resize(256)
        ...
        self.logger.info("Test and Train datasets loaded.")
        self.logger.info("Device subset generated.")
        # Load files for normalization
        if is_use_noniid_filter:  # If using Non-IID filter
            train_mean_file_path = os.path.join(noniid_filter_path, f"mean_r{filter_rate:02d}_s{filter_seed:02d}.pt")
            train_std_file_path = os.path.join(noniid_filter_path, f"std_r{filter_rate:02d}_s{filter_seed:02d}.pt")
            mean_list = torch.load(train_mean_file_path)  # Get the mean and standard deviation of pixel values for each node
            std_list = torch.load(train_std_file_path)  # Get the mean and standard deviation of pixel values for each node
        ...
        self.logger.info("Loading of mean and std in train data finished")

        mean = mean_list[agent_index]
        mean = mean.tolist()
        std = std_list[agent_index]
        std = std.tolist()
        ...
        net = select_net(model_name, len(classes), n_middle, pretrained_path="./latest_self_model.pth").to(device)

        # 5.3 Define the optimization function
        # Optimizer for model ensemble
        optimizer = select_optimizer(model_name, net, optimizer_name, lr, momentum)

        pretrain_optimizer = select_optimizer(model_name, net, pretrain_optimizer_name, pretrain_lr, pretrain_momentum)
        scheduler = None
        # If using lr decay in training
        if use_scheduler:
            scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=scheduler_step, gamma=scheduler_rate)

        pretrain_scheduler = None
        # If using lr decay in pretraining
        if use_pretrain_scheduler:
            pretrain_scheduler = optim.lr_scheduler.StepLR(
                pretrain_optimizer,
                step_size=pretrain_scheduler_step,
                gamma=pretrain_scheduler_rate,
            )
        self.logger.info("Dataset and Model ready for training.")
        ## 6. Training Phase
        self.model = net
        ...
        self.model_sharing.update_model_instance(self.model, "ViT", epoch=0)
    ...
    def self_learn(self, five_digit_number_str: str = "99999", WAFL_LEARN=False) -> bool:
        ...
        try:
            ...
            self.train_accuracy = outputs[0]
            self.train_loss = outputs[2]
            self.logger.info(f"📉 Training Loss: {self.train_loss:.6f}")
            self.logger.info(f"📈 Training Accuracy: {self.train_accuracy:.4f}")
            self.test_accuracy = outputs[1]
            self.test_loss = outputs[3]
            self.logger.info(f"📉 Test Loss: {self.test_loss:.4f}")
            self.logger.info(f"📈 Test Accuracy: {self.test_accuracy:.4f}")
            self._save_results_to_csv(
                self.epoch_number, self.train_accuracy, self.train_loss, self.test_accuracy, self.test_loss
            )
            # Code for the Self-TRAIN epoch stage #
            # Code for the Self-TRAIN epoch stage #
            # Code for the Self-TRAIN epoch stage #
    ...
    def wafl_learn(self, five_digit_number_str: str) -> bool:
        ...
        try:
            ...
            if n_nbr:
                ...
                outputs = evaluate(self.model, self.test_loader, self.criterion, self.device)
                self.test_accuracy = outputs[0]
                self.test_loss = outputs[1]
                ...
                self.logger.info(f"📉 Test Loss (no exchange): {self.test_loss:.4f}")
                self.logger.info(f"📈 Test Accuracy (no exchange): {self.test_accuracy:.4f}")
                self._save_results_to_csv(
                    self.epoch_number, self.train_accuracy, self.train_loss, self.test_accuracy, self.test_loss
                )
                self.epoch_number += 1
                    ...
                    if sub_command == "SELF":
                        try:
                            ...
                            self.model_sharing.update_model_instance(
                                self.model_learning.model, "ViT", epoch=int(five_digit_number_str) - 1
                            )
                            conn.sendall("OK\r\n".encode("utf-8"))
                            return True
                        except Exception as e:
