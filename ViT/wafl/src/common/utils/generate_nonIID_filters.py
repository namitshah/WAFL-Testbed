...
if __name__ == "__main__":
    ...
    data_dir = os.path.normpath(os.path.join(current_path, "../../../dataset/common"))
    filename = os.path.join(data_dir, f"non-IID_filter/filter_r{ratio:02d}_s{randomseed:02d}.pt")
    meanfile = os.path.join(data_dir, f"non-IID_filter/mean_r{ratio:02d}_s{randomseed:02d}.pt")
    stdfile = os.path.join(data_dir, f"non-IID_filter/std_r{ratio:02d}_s{randomseed:02d}.pt")
    ...
    trainloader = torch.utils.data.DataLoader(trainset, batch_size=batch_size, num_workers=0, pin_memory=True)
