...
def evaluate_history(histories, cur_dir):
    ...
    with open(os.path.join(cur_dir, "log.txt"), "a") as f:
        ...
        for i in range(len(histories)):
            f.write(f"Initial Epoch (node{i}): Loss: {histories[i][0, 3]:.5f} Accuracy: {histories[i][0, 4]:.5f}\n")
            f.write(f"Final Epoch (node{i}): Loss: {histories[i][-1, 3]:.5f} Accuracy: {histories[i][-1, 4]:.5f}\n")
...
def show_image_labels(loader, classes, net, device, loaded_epoch, cur_dir, all_images):
    if all_images:
        ...
        row_num = len(loader.dataset) // 10 if len(loader.dataset) % 10 == 0 else len(loader.dataset) // 10 + 1
    else:
        plt.figure(figsize=(20, 15))
        row_num = loader.batch_size // 10 if loader.batch_size % 10 == 0 else loader.batch_size // 10 + 1
    index = 0
    for images, labels in loader:
        ...
        if not all_images:
            break
        index += 1
    plt.savefig(os.path.join(cur_dir, f"images/sample_images/si-epoch-{loaded_epoch}.png"))
...
def make_latent_space(y_tests, y_outputs, epoch, ls_path, cur_node):
    ...
    plt.gca().set_aspect("equal", "datalim")
    plt.colorbar(boundaries=np.arange(11) - 0.5).set_ticks(np.arange(10))
    plt.title(f"UMAP projection of features @ epoch={epoch:d}, node={cur_node}", fontsize=12)
    plt.xlabel("Component 1")
    plt.ylabel("Component 2")
    plt.savefig(ls_path, bbox_inches="tight")
