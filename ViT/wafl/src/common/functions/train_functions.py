...
class MyGPUdatasetFolder(datasets.DatasetFolder):
    ...
    def __init__(self, root, device, transform=None):
        super().__init__(root, loader=self.custom_loader, extensions=self.IMG_EXTENTIONS)
...
def pretrain(
    ...
    print(f"TACC: {train_acc} | VACC: {val_acc} | ATLS: {avg_train_loss} | AVLS: {avg_val_loss}")

    if scheduler != None:
        scheduler.step()
        print(scheduler.get_last_lr())
    return (train_acc, val_acc, avg_train_loss, avg_val_loss)
...
def show_dataset_contents(
    ...
    for i, label in enumerate(classes):
        ...
        files_list_train = [d for d in files_and_dirs_in_container_train if os.path.isfile(os.path.join(class_path_train, d))]
        ...
        files_list_test = [d for d in files_and_dirs_in_container_test if os.path.isfile(os.path.join(class_path_test, d))]
        num_test[i] = len(files_list_test)

    with open(os.path.join(result_path, "log.txt"), "w") as f:
        for i in range(len(num_test)):
            f.write(f"label: {classes[i]} train_data: {num_train[i]} test_data: {num_test[i]}\n")
...
def train_for_cmls(cur_dir, epoch, n, cur_time_index, classes, net, criterion, test_loader, device):
    n_val_acc = 0
    val_loss = 0
    n_test = 0
    ...
    if not (os.path.exists(normalized_cm_dir_path)) or os.path.isfile(normalized_cm_dir_path):
...
def select_optimizer(model_name, net, optimizer_name, lr, momentum=None):
    if optimizer_name == "SGD":
        if model_name == "vgg19_bn":
            optimizer = optim.SGD(net.classifier[6].parameters(), lr=lr, momentum=momentum)
        elif model_name == "resnet_152":
            optimizer = optim.SGD(net.fc.parameters(), lr=lr, momentum=momentum)
        elif model_name == "mobilenet_v2":
            optimizer = optim.SGD(net.classifier[1].parameters(), lr=lr, momentum=momentum)
