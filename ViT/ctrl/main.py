    ...
    def start_remote_process(self, experiment_id: str, ssh_password: str) -> bool:
        ...
        try:
            ...
            with paramiko.SSHClient() as ssh:
                ...
                if exit_status != 0:
                    error_msg = stderr.read().decode().strip()
                    raise RuntimeError(f"❌ Failed to install tcpdump: {error_msg}")
                else:
                    self.logger.info("✅ Tcpdump installation confirmed! ")
...
class ControlServer:
    ...
    def run_experiment(self, epochs: Dict[str, int], wafl_phase: Dict[str, Any], contact_pattern: str):
        ...
        try:
            ...
            self.logger.info("⏳ Waiting 120 seconds for agents to stabilize...")
            for i in range(12):
                self.logger.info(f"{120 - i * 10} seconds remain...")
                time.sleep(10)
    ...
    def _wait_for_all_agents_to_complete(self, current_epoch: int, poll_interval: int = 2.5, timeout: int = 3600):
