...
deploy_to_device() {
    ...
    if $SKIP_DATASET_DEPLOYMENT; then
        echo "📦 Skipping dataset/ deployment..."
    else
        ssh $SSH_OPTS $MUX_OPTS "$host" "rm -rf $DEPLOYMENT_LOCATION/$TARGET_NAME/dataset"
    fi &&

    ssh $SSH_OPTS $MUX_OPTS "$host" "\
    ...
    { $SKIP_DATASET_DEPLOYMENT || { [ "$(ls -A $TARGET_PATH/wafl/dataset/common 2>/dev/null)" ] && { ((++ERROR_CHECK)) || true; } &&
    ...
    { $SKIP_DATASET_DEPLOYMENT || { [ "$(ls -A $TARGET_PATH/wafl/dataset/$device_name 2>/dev/null)" ] && { ((++ERROR_CHECK)) || true; } &&
