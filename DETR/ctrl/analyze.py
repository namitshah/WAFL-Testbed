        ...
        if os.path.isdir(device_path) and device_id_str.isdigit():
            ...
            if os.path.exists(csv_path):
                try:
                    ...
                    df_train = df_wide[["epoch", "train_mAP", "train_loss"]].copy()
                    df_train.rename(columns={"train_mAP": "mAP", "train_loss": "loss"}, inplace=True)
                    df_train["phase"] = "train"

                    # Transform test data into long format
                    df_test = df_wide[["epoch", "test_mAP", "test_loss"]].copy()
                    df_test.rename(columns={"test_mAP": "mAP", "test_loss": "loss"}, inplace=True)
                    df_test["phase"] = "test"
        ...
        if os.path.isdir(device_path) and device_id_str.isdigit():
            ...
            if os.path.exists(csv_path):
                try:
                    ...
                    df_wide["inbound_gigabytes"] = df_wide["inbound_bytes"] / 1e9
                    df_wide["outbound_gigabytes"] = df_wide["outbound_bytes"] / 1e9
                    df_wide["total_gigabytes"] = df_wide["inbound_gigabytes"] + df_wide["outbound_gigabytes"]
...
def plot_network_curves(df: pd.DataFrame, output_dir: str, experiment_id: str, epoch_ranges: Optional[dict] = None):
    ...
    ax = plt.gca()
    pivot = df.pivot(index="epoch", columns="device_id", values="total_gigabytes")
    ...
    plt.ylabel("Inbound + Outbound Traffic (GB)", fontsize=16)
    ...
    df_cumulative["total_gigabytes"] = df.groupby("device_id")["total_gigabytes"].cumsum()
    pivot = df_cumulative.pivot(index="epoch", columns="device_id", values="total_gigabytes")
    epochs = pivot.index.values
    devices = pivot.columns
    traffic_values = pivot.fillna(0).values.T
    colors = sns.color_palette("icefire", n_colors=len(devices))
    ...
    for device_id in device_id_list:
        ...
        traffic_values = df_dev[["inbound_gigabytes", "outbound_gigabytes"]].fillna(0).to_numpy().T
        ...
        ax.set_ylabel("Network Traffic (GB)", fontsize=16)
        ax2 = ax.twinx()
        sns.lineplot(
    ...
    for device_id in device_id_list:
        ...
        df_dev["inbound_gigabytes"] = df_dev["inbound_gigabytes"].cumsum()
        df_dev["outbound_gigabytes"] = df_dev["outbound_gigabytes"].cumsum()
        epochs = df_dev["epoch"].to_numpy()
        traffic_values = df_dev[["inbound_gigabytes", "outbound_gigabytes"]].fillna(0).to_numpy().T
        colors = [sns.color_palette("icefire", as_cmap=True)(0.15), sns.color_palette("icefire", as_cmap=True)(0.85)]
        ax.stackplot(epochs, traffic_values, labels=["Inbound", "Outbound"], colors=colors, linewidth=0)
...
def plot_learning_curves(df: pd.DataFrame, output_dir: str, experiment_id: str, epoch_ranges: Optional[dict] = None):
    ...
    sns.lineplot(ax=ax_acc, data=df, x="epoch", y="mAP", hue="phase", errorbar="sd")
    ax_acc.set_title(f"Experiment ID: {experiment_id}", fontsize=20)
    ax_acc.set_ylabel("mAP @ 0.5 IOU", fontsize=16)
    ...
    save_path_acc = os.path.join(output_dir, "1_mAP_curve.png")
    plt.savefig(save_path_acc)
    plt.close()
    print(f"  📈 Saved average mAP curve to: {save_path_acc}")
    ...
    plt.figure(figsize=(12, 7))
    ax_indiv = plt.gca()
    sns.lineplot(data=df_test, x="epoch", y="mAP", hue="device_id", palette="viridis_r", legend="full")
    plt.title(f"Individual Device Test mAP @ 0.5 IOU vs. Epoch\n(Experiment: {experiment_id})", fontsize=20)
    plt.ylabel("Test mAP @ 0.5 IOU", fontsize=16)
    plt.xlabel("Epoch", fontsize=16)
    ...
    plt.tight_layout()
    save_path = os.path.join(output_dir, "2_individual_test_mAP.png")
    plt.savefig(save_path, bbox_inches="tight")
    plt.close()
...
def main():
    ...
    mAP_mean = (
        aggregated_df[
            (aggregated_df["phase"] == "test") & (aggregated_df["epoch"] > aggregated_df["epoch"].max() - final_block)
        ]["mAP"].mean()
        * 100
    )
    mAP_sd = (
    ...
    print(f"---Mean of Final Test mAP Values (Last {final_block} Epochs): {mAP_mean}%")
    print(f"---SD of Final Test mAP Values (Last {final_block} Epochs): {mAP_sd}%")
    print("\n🎉 --- Analysis complete! ---")
