...
oid_labels = ["Person", "Bicycle", "Car", "Table", "Door", "Fire hydrant", "Waste container", "Ball", "Cat", "Drink"]
...
def plot_finetuned_results(pil_img, image_name, threshold, prob=None, boxes=None, labels=None):
    ...
    if prob is not None and boxes is not None:
        for p, (xmin, ymin, xmax, ymax), c in zip(prob, boxes.tolist(), colors):
            ax.add_patch(plt.Rectangle((xmin, ymin), xmax - xmin, ymax - ymin, fill=False, color=c, linewidth=4))
            cl = p.argmax()
            print(labels, p)
